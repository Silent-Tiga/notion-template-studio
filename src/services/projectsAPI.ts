...
export async function getProject(id: string): Promise<Project> {
  const res = await fetch(`${endpoint}?id=${encodeURIComponent(id)}`, { method: 'GET', headers: await getAuthHeaders() });
...
export async function createProject(params: { siteTitle: string; rootDatabaseId?: string; plan?: ProjectPlan; mappings?: Record<string, any> }): Promise<Project> {
  const res = await fetch(endpoint, { method: 'POST', headers: await getAuthHeaders(), body: JSON.stringify(params) });
...
export async function updateProject(id: string, changes: Partial<Project>): Promise<Project> {
  const res = await fetch(endpoint, { method: 'PATCH', headers: await getAuthHeaders(), body: JSON.stringify({ id, ...changes }) });
...
export async function deleteProject(id: string): Promise<void> {
  const res = await fetch(`${endpoint}?id=${encodeURIComponent(id)}`, { method: 'DELETE', headers: await getAuthHeaders() });
